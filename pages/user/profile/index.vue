  ...
  <div class="main">
    ...
    <form @submit.prevent="submitProfile">
      ...
      <v-row>
        ...
        <v-col cols="12" md="6">
          <v-btn lg outlined color="error" disabled block>
            Discard
          </v-btn>
        </v-col>
      </v-row>
...
const userType = computed(() => userData.user.value ? userData.user.value.group_id : null);
...
const firstNameRules = [
  ...
  (v: string) => /^[a-zA-Z\s]+$/.test(v) || 'First name can only contain letters and spaces',
];
...
const lastNameRules = [
  ...
  (v: string) => /^[a-zA-Z\s]+$/.test(v) || 'Last name can only contain letters and spaces',
];
...
const getUserInfo = async () => {
  try {
    const apiUrl = userType.value === 5 ? "/api/v1/teachers/dashboard" : "/api/v1/students/dashboard";
...
watch(() => form.first_name, (val) => validateField('first_name', val));
watch(() => form.last_name, (val) => validateField('last_name', val));
watch(() => form.gender, (val) => validateField('gender', val));
watch(() => form.state, (val) => validateField('state', val));
watch(() => form.area, (val) => validateField('area', val));
watch(() => form.level, (val) => validateField('level', val));
watch(() => form.grade, (val) => validateField('grade', val));
watch(() => form.school, (val) => validateField('school', val));
watch(() => username.value, (val) => validateField('username', val));
