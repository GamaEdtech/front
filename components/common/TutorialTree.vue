    ...
    <ul class="ml-4">
      <li v-for="item in items" :key="item.id" class="mb-2">
        <div class="d-flex align-center" :class="{
          'bg-teal text-white': isSubOpen(item.id) && !isRootLevel}" style="cursor: pointer" @click="menuClicked(item.id)">
          <v-icon v-if="isRootLevel" color="teal" size="small" class="mr-2">
            {{ isOpen(item.id) ? 'mdi-chevron-down' : 'mdi-chevron-right' }}
          </v-icon>
          <v-icon :color="isSubOpen(item.id) && !isRootLevel ? 'white' : 'teal'"  size="small" class="mr-2" :class="{
          ...
          <div>
            <NuxtLink
              v-if="item.tutorials && item.tutorials.length==1 && !isRootLevel" @click="subTreeClicked(item.id)"
...
let activeMenu = ref(null); // Track the currently MainMenuBtn

let activeSubMenu = ref(null) // Track the currently SubMenuBtn
...
const initOpenMenu =() => {
      for (let index in props.items){
        for(let i in props.items[index].chapters){
