...
const fetchInitialData = async () => {
  ...
  if (route.query.base) {
    const gradeIndex = gradeList.value.data.findIndex(
      (x) => x.id === route.query.base
    );
...
const openDialog = () => {
  ...
  setTimeout(() => {
    if (side_filter.value) {
      ...
      if (route.query.type == "test")
        side_filter.value.setFilter("paper_type", paperTypeList.value?.data);
    }
  }, 100);
};
      ...
      <v-row justify="center" class="d-block d-md-none">
        ...
        >
          <v-card style="background-color: #f5f5f5; padding: unset !important">
            ...
            <v-card-text
              style="background-color: #f5f5f5; padding: unset !important"
            >
              <search-filter
                ref="side_filter"
                class="mx-3"
