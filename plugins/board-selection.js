...
export default ({ app, store, route }) => {
  ...
  if (process.client) {
        ...
        if (response.data && Array.isArray(response.data.data)) {
          ...
          if (boardId === "default") {
            ...
            const defaultBoard = boards.find(
              (board) => String(board.id) === String(DEFAULT_BOARD_ID)
            );
            ...
            const boardDetails = boards.find(
    ...
    const createBoardObject = (boardId, boardDetails = null) => {
      ...
      if (boardDetails) {
        boardObj.title =
          boardDetails.title || boardDetails.name || boardDetails.id;
        boardObj.name =
          boardDetails.name || boardDetails.title || boardDetails.id;
        ...
        if (boardCache[boardId]) {
          boardObj.title =
            boardCache[boardId].title || boardCache[boardId].name || boardId;
          boardObj.name =
            boardCache[boardId].name || boardCache[boardId].title || boardId;
